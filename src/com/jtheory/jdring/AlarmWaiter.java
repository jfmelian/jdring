...
/**
 * This class manages the thread which sleeps until the next alarm. Methods are synchronized to prevent interference from the AlarmWaiter thread and external
 * threads.
 * 
...
public class AlarmWaiter implements Runnable {
     ...
     * 
     * @param _sleep_until
     *            the new time to sleep until.
     */
    public synchronized void update(long _sleep_until) {
        this.sleepUntil = _sleep_until;
        Logger.debug("Update for " + _sleep_until); // timeToSleep);
        Logger.debug("calling notify() to update thread wait timeout");
        notify();
    }
        ...
        while (!shutdown) {
            try {
                ...
                if (sleepUntil >= 0 && (sleepUntil - System.currentTimeMillis() < 1000)) {
                    ...
                    sleepUntil = -1;
                    Logger.debug("notifying manager to ring next alarm");
                    mgr.ringNextAlarm();
                }
